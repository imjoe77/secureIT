...
router.get('/logs', async (req, res) => {
  ...
  const total = await db.prepare(`SELECT COUNT(*) as count FROM audit_log WHERE tenant_id = ?`).get(req.user.tenantId);
...
router.delete('/logs', async (req, res) => {
  const db = getConnection();
  await db.prepare('DELETE FROM audit_log WHERE tenant_id = ?').run(req.user.tenantId);
...
router.get('/stats', async (req, res) => {
  ...
  const totalRequests = await db.prepare('SELECT COUNT(*) as count FROM audit_log WHERE tenant_id = ?').get(tid);
  const allowed = await db.prepare("SELECT COUNT(*) as count FROM audit_log WHERE tenant_id = ? AND decision = 'ALLOW'").get(tid);
  const denied = await db.prepare("SELECT COUNT(*) as count FROM audit_log WHERE tenant_id = ? AND decision = 'DENY'").get(tid);

  const recentDenials = await db.prepare(`
  ...
  const topDeniedPerms = await db.prepare(`
...
router.get('/rules', async (req, res) => {
  const db = getConnection();
  const rules = await db.prepare('SELECT * FROM firewall_rules WHERE tenant_id IS NULL OR tenant_id = ? ORDER BY created_at').all(req.user.tenantId);
...
router.post('/rules', async (req, res) => {
  ...
  await db.prepare('INSERT INTO firewall_rules (id, name, description, rule_type, config, tenant_id) VALUES (?, ?, ?, ?, ?, ?)').run(
...
router.get('/users', async (req, res) => {
  ...
  const users = await db.prepare(`
    ...
    WHERE u.tenant_id = ?
    GROUP BY u.id, t.name ORDER BY u.username
...
router.post('/trusted-devices', async (req, res) => {
  ...
  // Find the role
  const role = await db.prepare('SELECT id, name FROM roles WHERE name = ? AND tenant_id = ?')
  ...
  const existing = await db.prepare('SELECT id FROM trusted_devices WHERE ip_address = ? AND role_id = ? AND is_active = TRUE')
...
router.delete('/trusted-devices/:id', async (req, res) => {
  const db = getConnection();
  const device = await db.prepare('SELECT * FROM trusted_devices WHERE id = ? AND tenant_id = ?')
  ...
  await db.prepare('UPDATE trusted_devices SET is_active = FALSE WHERE id = ?').run(req.params.id);
...
router.get('/threats', async (req, res) => {
  try {
    const result = await analyzeThreats(req.user.tenantId);
    res.json(result);
  } catch (err) {
...
router.delete('/threats/:userId', async (req, res) => {
