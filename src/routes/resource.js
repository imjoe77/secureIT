...
router.get('/reports', authorize('VIEW', 'REPORTS'), async (req, res) => {
...
// ─── UPDATE REPORTS ───────────────────────────────────────────────────────────
router.put('/reports', authorize('UPDATE', 'REPORTS'), async (req, res) => {
  res.json({
    status: 'ALLOWED',
...
// ─── DELETE REPORTS ───────────────────────────────────────────────────────────
router.delete('/reports', authorize('DELETE', 'REPORTS'), async (req, res) => {
  res.json({
    status: 'ALLOWED',
...
router.get('/documents', isAdmin, async (req, res) => {
  ...
  const documents = await db.prepare(`
    SELECT d.id, d.title, d.classification, d.created_at,
           ...
           string_agg(r.name, ',') as owner_roles
    FROM documents d
...
router.get('/shares', isAdmin, async (req, res) => {
  const db = getConnection();
  const shares = await db.prepare(`
    SELECT ds.document_id, ds.user_id, ds.access_level, ds.granted_at,
           ...
           string_agg(r.name, ',') as recipient_roles
    ...
    LEFT JOIN roles r ON ur.role_id = r.id
    WHERE ds.tenant_id = ?
    GROUP BY ds.document_id, ds.user_id, ds.access_level, ds.granted_at, d.title, d.classification, owner.username, recipient.username
...
router.post('/share', isAdmin, async (req, res) => {
  ...
  const doc = await db.prepare('SELECT id, title, owner_id, tenant_id FROM documents WHERE id = ? AND tenant_id = ?').get(documentId, req.user.tenantId);
  ...
  const target = await db.prepare('SELECT id, username, tenant_id FROM users WHERE id = ? AND tenant_id = ?').get(targetUserId, req.user.tenantId);
  ...
  const existing = await db.prepare('SELECT 1 FROM document_shares WHERE document_id = ? AND user_id = ?').get(documentId, targetUserId);
  if (existing) return res.status(409).json({ error: 'ALREADY_SHARED', message: `${target.username} already has access to '${doc.title}'.` });

  // 5. Create the share
  await db.prepare('INSERT INTO document_shares (document_id, user_id, tenant_id) VALUES (?, ?, ?)').run(documentId, targetUserId, req.user.tenantId);
...
router.delete('/share', isAdmin, async (req, res) => {
  const { documentId, targetUserId } = req.body;
  if (!documentId || !targetUserId) {
    return res.status(400).json({ error: 'VALIDATION_ERROR', message: 'documentId and targetUserId are required.' });
  }
  ...
  await db.prepare('DELETE FROM document_shares WHERE document_id = ? AND user_id = ?').run(documentId, targetUserId);
