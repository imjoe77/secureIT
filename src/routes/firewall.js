...
router.patch('/rules/:id/toggle', (req, res) => {
  try {
    ...
    const rule = db.prepare('SELECT is_active FROM firewall_rules WHERE id = ?').get(id);
    if (!rule) return res.status(404).json({ error: 'RULE_NOT_FOUND' });

    const newState = rule.is_active === 1 ? 0 : 1;
    db.prepare('UPDATE firewall_rules SET is_active = ? WHERE id = ?').run(newState, id);
